...
<body class="antialiased">
    <div class="flex relative min-h-screen bg-gray-100 dark:bg-gray-900 sm:pt-0">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <h1>Muokataan muistiinpanoa: {{ $note->heading }}</h1>
                ...
                <div class="flex flex-col w-full">
                    ...
                    <form method="post">
                        ...
                        <div class="flex flex-col mb-4">
                            <label>Otsikko</label>
                            <input class="border border-black p-3" type="text" name="heading" value="{{ $note->heading }}" placeholder="Otsikko">
                        </div>

                        <div class="flex flex-col mb-4">
                            <label>Sisältö</label>
                            <textarea class="border border-black p-3 h-40" name="body" placeholder="">{{ $note->body }}</textarea>
                        </div>
                    ...
                    <form action="{{ route('notes.delete', ['note' => $note->id]) }}" method="post" id="deleteForm">
                        {{ csrf_field() }}
                        {{ method_field('delete') }}
                    </form>
                </div>
